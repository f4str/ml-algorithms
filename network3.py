...
class NeuralNetwork:
	def __init__(self):
		...
		self.weights = {
			'w1': tf.Variable(tf.random_normal([self.num_input, self.n_hidden_1])),
	...
	def build(self):
		...
		self.layer1 = tf.matmul(self.x, self.weights['w1']) + self.biases['b1']
		self.layer2 = tf.matmul(self.layer1, self.weights['w2']) + self.biases['b2']
		self.logits = tf.matmul(self.layer2, self.weights['out']) + self.biases['out']
